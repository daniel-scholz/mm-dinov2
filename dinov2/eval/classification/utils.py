...
from dinov2.eval.utils import Model3DWrapper
...
class LinearClassifier(nn.Module):
    ...
    def __init__(
        self, out_dim, use_n_blocks, use_avgpool, num_classes=1000, is_3d=False
    ):
        super().__init__()
    ...
    def forward(self, x):
        output = (
            torch.stack(  # If 3D, take average of all slices.
                [
                    create_linear_input(image, self.use_n_blocks, self.use_avgpool)
                    for image in x
                ]
            )
            .mean(dim=0)
        ...
        for avgpool in avgpools:
            for _lr in learning_rates:
                ...
                linear_classifiers_dict[
                    f"linear:blocks={n}:avgpool={avgpool}:lr={lr:.10f}".replace(
                        ".", "_"
                    )
                ] = linear_classifier
